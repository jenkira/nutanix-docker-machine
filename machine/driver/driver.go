...
func (d *NutanixDriver) GetCreateFlags() []mcnflag.Flag {
	return []mcnflag.Flag{
		mcnflag.StringFlag{EnvVar: "NUTANIX_USERNAME", Name: "nutanix-username", Usage: "Nutanix management username"},
		mcnflag.StringFlag{EnvVar: "NUTANIX_PASSWORD", Name: "nutanix-password", Usage: "Nutanix management password"},
		mcnflag.StringFlag{EnvVar: "NUTANIX_ENDPOINT", Name: "nutanix-endpoint", Usage: "Nutanix management endpoint ip address/FQDN"},
		mcnflag.StringFlag{EnvVar: "NUTANIX_PORT", Name: "nutanix-port", Usage: "Nutanix management endpoint port (default: 9440)", Value: "9440"},
		mcnflag.BoolFlag{EnvVar: "NUTANIX_INSECURE", Name: "nutanix-insecure", Usage: "Explicitly allow the provider to perform insecure SSL requests"},
		mcnflag.StringFlag{EnvVar: "NUTANIX_CLUSTER", Name: "nutanix-cluster", Usage: "Nutanix Cluster to install VM on"},
		mcnflag.IntFlag{EnvVar: "NUTANIX_VM_MEM", Name: "nutanix-vm-mem", Usage: "Memory in MB of the VM to be created", Value: defaultVMMem},
		mcnflag.IntFlag{EnvVar: "NUTANIX_VM_CPUS", Name: "nutanix-vm-cpus", Usage: "Number of VCPUs of the VM to be created", Value: defaultVCPUs},
		mcnflag.IntFlag{EnvVar: "NUTANIX_VM_CORES", Name: "nutanix-vm-cores", Usage: "Number of cores per VCPU of the VM to be created", Value: defaultCores},
		mcnflag.BoolFlag{EnvVar: "NUTANIX_VM_CPU_PASSTHROUGH", Name: "nutanix-vm-cpu-passthrough", Usage: "Enable passthrough the host's CPU features to the newly created VM"},
		mcnflag.StringSliceFlag{Name: "nutanix-vm-network", Usage: "The name of the network to attach to the newly created VM"},
		mcnflag.StringFlag{EnvVar: "NUTANIX_VM_IMAGE", Name: "nutanix-vm-image", Usage: "The name of the VM disk to clone from, for the newly created VM"},
		mcnflag.IntFlag{EnvVar: "NUTANIX_VM_IMAGE_SIZE", Name: "nutanix-vm-image-size", Usage: "Increase the size of the template image", Value: 0},
		mcnflag.StringSliceFlag{Name: "nutanix-vm-categories", Usage: "The name of the categories who will be applied to the newly created VM"},
		mcnflag.StringFlag{EnvVar: "NUTANIX_STORAGE_CONTAINER", Name: "nutanix-storage-container", Usage: "The UUID of the storage container", Value: ""},
		mcnflag.IntFlag{EnvVar: "NUTANIX_DISK_SIZE", Name: "nutanix-disk-size", Usage: "The size of the attached disk", Value: 0},
		mcnflag.StringFlag{EnvVar: "NUTANIX_CLOUD_INIT", Name: "nutanix-cloud-init", Usage: "Cloud-init configuration"},
		mcnflag.BoolFlag{EnvVar: "NUTANIX_VM_SERIAL_PORT", Name: "nutanix-vm-serial-port", Usage: "Attach a serial port to the newly created VM (type Null)"},
		mcnflag.StringFlag{EnvVar: "NUTANIX_PROJECT", Name: "nutanix-project", Usage: "The name of the project to assign the VM", Value: ""},
		mcnflag.StringFlag{EnvVar: "NUTANIX_BOOT_TYPE", Name: "nutanix-boot-type", Usage: "The boot type of the VM (legacy or uefi)", Value: defaultBootType},
		mcnflag.IntFlag{EnvVar: "NUTANIX_TIMEOUT", Name: "nutanix-timeout", Usage: "Timeout for Nutanix operations (in seconds)", Value: 300},
		mcnflag.StringSliceFlag{Name: "nutanix-vm-gpu", Usage: "The list of GPU devices to attach to the newly created VM"},
		mcnflag.StringFlag{Name: "nutanix-vm-description", Usage: "The description of the newly created VM"},
	}
}
